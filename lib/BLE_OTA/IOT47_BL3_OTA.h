...
int iot47_ota_task(uint8_t *rxValue, uint8_t len)
{
                ...
                if (ota_cmd != 0)
                {
                    ...
                    for (int i = 0; i < 20; i++)
                    {
                        if ((ota_cmd[20 + i] == '\r') || (ota_cmd[20 + i] == '\n'))
                        {
                            ...
                            if (begin_callback != 0)
                                begin_callback(ota_fw_counter, ota_fw_size);
    ...
    else if (ota_state == OTA_DOWNLOADDING)
    {
        ...
        if (packet == ota_download_paket)
        {
            ...
            {
                couter_process = 0;
                if (proces_callback != 0)
                    proces_callback(ota_fw_counter, ota_fw_size);
            }
            if (ota_fw_counter == ota_fw_size)
            {
                ...
                if (end_callback != 0)
                    end_callback(ota_fw_counter, ota_fw_size);
                UpdateRun();
                return 3;
            }
        }
        else
        {
            ...
            OTA_BLECharacteristic->notify();
            if (error_callback != 0)
                error_callback(ota_fw_counter, ota_fw_size);
        }
        return 2;
    }
    return 0;
